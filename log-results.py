...
query3 = (" WITH T1 AS"
          " ( SELECT date(time) AS date1, floor(100.0*sum(case log.status when '200 OK'  then 0 else 1 end)/count(log.status)) "
...
def Print_Query(result):
    for i in range(len(result)):
        print("\t" + "%s - %d" % (result[i][0], result[i][1]) + " views for Question above")
...
Print_Query(Perform_Query(query3))
